    ...
    function renderCards() {{
      ...
      els.gallery.innerHTML = filtered.map((item) => {{
        ...
        const ariaLabel = isSold
          ? `${{escapeHtml(item.machineName)}} は売約済み`
          : `${{escapeHtml(item.machineName)}} の写真を開く`;

        return `
          <article class="card${{soldClass}}">
            <button class="card-button" type="button" data-no="${{escapeHtml(item.no)}}" aria-label="${{ariaLabel}}"${{disabledAttr}}>
              ${photoBlock}
              <div class="card-body">
